...
class TextMatcher:
    ...
    def detect_split_sentences(self, similarity_matrix, threshold=0.3, split_threshold=0.8):
        split_sentences = []
        for i in range(similarity_matrix.shape[0]):
            ...
            if len(potential_splits) > 1:
                ...
                if total_similarity > split_threshold:
                    split_sentences.append((i, (potential_splits[0], potential_splits[-1])))
        return split_sentences

    def analyze_changes(self, original_sentences, revised_sentences, matches, split_sentences):
        changes = []
        matched_original = set(m[0] for m in matches)
        matched_revised = set(m[1] for m in matches)

        for o, r in matches:
            if any(o == s[0] and r >= s[1][0] and r <= s[1][1] for s in split_sentences):
